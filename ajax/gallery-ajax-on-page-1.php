...
<?php
$sql=mysqli_query($con,"SELECT count(*) as nb_photo_Baptemes  FROM `galerie` WHERE `type`='Baptemes' ");
$cnt=1;
...
<p class="text-center text-1"><?php echo $row['nb_photo_Baptemes'];?> photos</p>
...
<div class="owl-carousel custom-arrows-style-2 custom-nav-inside-center" data-plugin-options="{'items': 1, 'margin': 0, 'loop': true, 'nav': true, 'dots': true}">
	...
	<?php
$sql=mysqli_query($con,"SELECT * FROM `galerie` WHERE `type`='Baptemes'  order by 	id_photo  desc ");
$cnt=1;
while($row=mysqli_fetch_array($sql))
{
?>
